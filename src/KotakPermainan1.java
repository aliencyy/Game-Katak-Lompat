...
public class KotakPermainan1 {
     ...
     * @param jm
     */
    public KotakPermainan1(int j, int jk, int jm){
        this.jumlahKotak = j;
        this.jumlahKoin = jk;
    ...
    private void inisialisasiKotak(){
        ...
        Koin1 k1 = new Koin1(10);
        Monster1 m1 = new Monster1(-5, "Cyclops");
        ...
        for(int i = 0; i < jumlahKoin; i++){
            boardGame[acakKoin[i]].addKoin(k1);
        }

        for(int i = 0; i < jumlahMonster; i++){
            boardGame[acakMonster[i]].addMonster(m1);
        }
    }
